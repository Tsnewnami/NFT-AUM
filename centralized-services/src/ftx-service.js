...
class FtxClient {
  ...
  getOrderHistory(market, side, orderType, start_time, end_time) {
    return this._get("orders/history", {
      market,
      side,
      orderType,
      start_time,
  ...
  getConditionalOrderHistory(
    ...
    return this._get("conditional_orders/history", {
      market,
      side,
      type,
      orderType,
  ...
  modifyOrderByClientId(clientOrderId, price, size, clientId) {
    return this._post(`orders/by_client_id/${clientOrderId}/modify`, {
      size,
      price,
      clientId,
    });
